...
<div class="card">
  ...
  <div class="stats-row">
    ...
    <div class="stat"><span class="stat-label">Avg Price<sup>‡</sup></span><span class="stat-value">{avg_price}</span></div>
...
def _card_html(row, demo=False) -> str:
    ...
    avg_price = f"{row['Avg Price']:.3f}" if "Avg Price" in row and pd.notna(row["Avg Price"]) else "—"
...
<div class="wrap">
  ...
  <footer>
    ...
    <br><sup>‡</sup>Avg Price is today's high/low/close typical price (a single-day VWAP proxy, since
    daily bars don't have intraday ticks to compute a true running VWAP for just today).
