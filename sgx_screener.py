...
def apply_ta_filters(res: StockResult, hist: pd.DataFrame):
    ...
    # Today's single-day volume-weighted average price -- the standard proxy
    # for "the day's VWAP" when only a daily OHLCV bar is available (no
    # intraday ticks to compute a true running VWAP for just today).
    res.avg_price = float((hist["High"].iloc[-1] + hist["Low"].iloc[-1] + hist["Close"].iloc[-1]) / 3)
...
def run_screen(universe=None, min_fa_score=1, min_ta_score=1) -> pd.DataFrame:
    ...
    df = pd.DataFrame([{
        ...
        "Avg Price": round(r.avg_price, 3) if not np.isnan(r.avg_price) else None,
